...
class Agendapdumum extends CI_controller
{
  function __construct()
  {
	   ...
	   $this->load->model('m_agendapdumum');
  }
  ...
  public function index()
  {
    ...
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawai($id) : $this->m_agendapdumum->agenda_pegawai();
	...
	$x = array('judul' =>'Agenda Kegiatan PD Umum',
			  'datacount' => $datacount,
              'data'  =>$data); 
    tpl('admin/agendapdumum/agenda',$x);
  }
  ...
  public function index2()
  {
    ...
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawai($id) : $this->m_agendapdumum->agenda_pegawai();
	$data3 = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawai($idm2) : $this->m_agendapdumum->cari_agenda_pegawai($idm2);
	$data2 = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawaim2($idm2) : $this->m_agendapdumum->cari_agenda_pegawaim2($idm2);
	...
	{
	   $idman=$dataman['id_agenda'];
	   $dataman=$this->db->query("SELECT * from agendapdumum WHERE id_agenda='$idman'")->row();
	...
	$x = array('judul' =>'Agenda Kegiatan',
			  ...
			  'data2'  =>$dataman2,
              'data'  =>$data); 
    tpl('admin/agendapdumum/agenda2',$x);
  ...
  public function agendacalendar()
  {
    ...
    tpl('admin/agendapdumum/agendacalendar',$x);
  }
  ...
  public function myagendacalendar()
  {
    ...
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawai($idm2) : $this->m_agendapdumum->cari_agenda_pegawai($idm2);
	$data2 = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawaim2($idm2) : $this->m_agendapdumum->cari_agenda_pegawaim2($idm2);
	$data3 = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawai($idm2) : $this->m_agendapdumum->agenda_pegawai();
	...
	{
      tpl('admin/agendapdumum/myagendacalendar',$x);
	}
	else
	{
	  tpl('admin/agendapdumum/mysagendacalendar',$x);
	}
  ...
  public function verifikasi_agenda()
  {
    ...
    tpl('admin/agendapdumum/agendaverifikasi',$x);
  }
  ...
  {
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawai($id) : $this->m_agendapdumum->agenda_pegawai_disposisi();
    ...
    tpl('admin/agendapdumum/agendadisposisi',$x);
  }
  ...
  {
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->kegiatan_pegawai($id_agenda) : $this->m_agendapdumum->kegiatan_pegawai($id_agenda);
	
	$sql=$this->db->get_where('agendapdumum',array('id_agenda'=>$id_agenda))->row_array(); 
    ...
    $x = array('judul' =>'Daftar Kegiatan Agenda Pegawai',
				  ...
				  'lokasi_agenda'    => $sql['lokasi_agenda'],
              	  'data'  =>$data); 
    tpl('admin/agendapdumum/agendakegiatan',$x);
  }
  ...
  public function verifikasiagenda_kegiatan($id_agenda)
  {
    ...
    $x = array('judul' =>'Daftar Kegiatan Agenda Pegawai',
	           'idm'=>$id_agenda,
				  ...
				  'lokasi_agenda'    => $sql['lokasi_agenda'],
              'data'  =>$data); 
    tpl('admin/agendapdumum/verifikasiagendakegiatan',$x);
  }
  ...
  {
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->disposisikegiatan_pegawai($id_agenda) : $this->m_agendapdumum->disposisikegiatan_pegawai($id_agenda);
    ...
    $x = array('judul' =>'Disposisi Daftar Kegiatan Agenda Pegawai',
	           'idm'=>$id_agenda,
				  ...
				  'lokasi_agenda'    => $sql['lokasi_agenda'],
              'data'  =>$data); 
    tpl('admin/agendapdumum/disposisiagendakegiatan',$x);
  }
  ...
  public function setujui_kegiatan($id,$idm)
  {
	    ...
	    if($sql['status_kegiatan']==0)
		{
		  ...
		  redirect(base_url('agendapdumum/verifikasiagenda_kegiatan/'.$idm));
		}
		else
		{
		  redirect(base_url('agendapdumum/verifikasiagenda_kegiatan/'.$idm));
  ...
  public function tolak_kegiatan($id='',$idm='')
  {
		...
		{   	  
			  $x = array('judul'        =>'Tolak Data Kegiatan' ,
				...
				{
				 tpl('admin/agendapdumum/tolakkegiatan_form',$x);
				}
		...
		{   	  
			  if($sql['status_kegiatan']==0)
			  {
			   ...
			   else
			   {
				...
				{
				 tpl('admin/agendapdumum/wakilkegiatan_form',$x);
				}
  ...
  {
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawai($id) : $this->m_agendapdumum->agenda_pegawai_pending();
    $x = array('judul' =>'Pending Agenda Pegawai',
              'data'  =>$data); 
    tpl('admin/agendapdumum/agendapending',$x);
  }
  
  public function agenda_approved()
  {
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawai($id) : $this->m_agendapdumum->agenda_pegawai_approved();
    $x = array('judul' =>'Apporved Agenda Pegawai',
              'data'  =>$data); 
    tpl('admin/agendapdumum/agendaapproved',$x);
  }
  
  public function agenda_reject()
  {
    $id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
    $data = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawai($id) : $this->m_agendapdumum->agenda_pegawai_reject();
    $x = array('judul' =>'Reject Agenda Pegawai',
              'data'  =>$data); 
    tpl('admin/agendapdumum/agendareject',$x);
  }
  ...
  public function jabatan()
  {
   ...
   tpl('admin/agendapdumum/jabatan',$x);
  }
  ...
  {
	  $x = array('judul'        => 'Tambah Data Agenda PD UMUM' ,
				  'aksi'        => 'tambah',
				  'tanggal'=> "",
				  'jam_mulai'=> "",
				  'jam_selesai'=> "",
		...
		if(isset($_POST['kirim']))
		{
			...
			$querydatajumdata=0;
			$querydatajum = $this->db->query("SELECT * from agendapdumum WHERE id_pegawai='$idpegawai' AND tanggal='$tanggal' AND (jam_mulai >= '$jammulai2' AND jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2') ");
			...
			if(!empty($datapegawai))
			{
				foreach($datapegawai as $datapegikut)
				{
					...
					$querydatajum3 = $this->db->query("SELECT * from agendapdumum WHERE id_pegawai='$datapegikut' AND tanggal='$tanggal' AND (jam_mulai >= '$jammulai2' AND jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2') ");
			...
			if($querydatajumdata>0 || $jumdatapegawai>0)
			{
					 ...
					 {		 $pegawai='';
						...
						$this->session->set_flashdata('pesan',$pesan);
						redirect(base_url('agendapdumum/agenda_tambah',$x));
			}
			else
			{
				$inputData=array(
				...
				$cek=$this->db->insert('agendapdumum',$inputData);
				...
				{	
					...
					$this->session->set_flashdata('pesan',$pesan);
					redirect(base_url('agendapdumum/index'));
				}
				else
				{
				echo "ERROR input Data";
		...
		{
			 tpl('admin/agendapdumum/agenda_form',$x);
	  ...
	  $x = array('judul'        => 'Tambah Data Agenda' ,
		...
		if(isset($_POST['kirim']))
		{
		  ...
		  if($cek)
		  {
				...
				$this->session->set_flashdata('pesan',$pesan);
				redirect(base_url('agendapdumum/agenda_kegiatan/'.$idm));
		  }
		  else
		  {
		   echo "ERROR input Data";
		...
		{
		 tpl('admin/agendapdumum/kegiatan_form',$x);
		}
  }
  ...
  {
	  $sql=$this->db->get_where('agendapdumum',array('id_agenda'=>$id))->row_array(); 
	  ...
	  $x = array('judul'        =>'Edit Data Agenda' ,
		...
		if(isset($_POST['kirim']))
		{
			...
			$querydatajumdata=0;
			$querydatajum = $this->db->query("SELECT * from agendapdumum WHERE id_pegawai='$idpegawai' AND tanggal='$tanggal' AND (jam_mulai >= '$jammulai2' AND jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2') AND id_agenda NOT IN($id)");
			$querydatajumdata=$querydatajum->num_rows();
			$querydatajumdatarow=$querydatajum->row();
			...
			if(!empty($datapegawai))
			{
				foreach($datapegawai as $datapegikut)
				{
					...
					$querydatajum3 = $this->db->query("SELECT * from agendapdumum WHERE id_pegawai='$datapegikut' AND tanggal='$tanggal' AND (jam_mulai >= '$jammulai2' AND jam_selesai <= '$jamselesai2' OR jam_mulai >='$jammulai3' AND jam_selesai <='$jamselesai2') AND id_agenda NOT IN($id)");
			...
			else
			{
				...
				$cek=$this->db->update('agendapdumum',$inputData,array('id_agenda'=>$id));
  ...
  public function agenda_detail($id='')
  {
	  ...
	  $x = array('judul'        =>'Detail Data Agenda' ,
	   ...
	   tpl('admin/agendapdumum/agenda_formdetail',$x);	
  }
  ...
  public function kegiatan_detail($id='')
  {
	   ...
	   tpl('admin/agendapdumum/kegiatan_formdetail',$x);	
  }
  ...
  {
	   $cek=$this->db->delete('agendapdumum',array('id_agenda'=>$id));
	   if ($cek) {
		$pesan='<div class="alert alert-success alert-dismissible">
					<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
					<h4><i class="icon fa fa-check"></i> Success!</h4>
  ...
  public function kegiatan_hapus($id='',$idm='')
  {
	...
	public function excel()
	{
		...
		$kolomhead = 0;
			...
			foreach ($this->db->query("SELECT * from agendapdumum d LEFT JOIN pegawai a ON(d.id_pegawai=a.id_pegawai)")->result() as $data) {
				  $kolombody = 0;
	...
	public function myexcel()
	{
		...
		$id   = ($this->session->userdata('level') == "pegawai") ? $this->session->userdata('id_pegawai') : $this->session->userdata('id_admin');
		$data = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawai($id) : $this->m_agendapdumum->agenda_pegawai();
		$data2 = ($this->session->userdata('level') == "pegawai") ? $this->m_agendapdumum->cari_agenda_pegawai($idm2) : $this->m_agendapdumum->cari_agenda_pegawai($idm2);
	...
	public function word()
	{
		...
		$data = array(
			'tbl_agenda_data' => $this->db->query("SELECT * from agendapdumum d LEFT JOIN pegawai a ON(d.id_pegawai=a.id_pegawai)")->result(),
			'start' => 0
		);
		
		$this->load->view('admin/agendapdumum/tbl_agenda_doc',$data);
	}
